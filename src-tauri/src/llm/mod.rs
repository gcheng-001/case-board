...
mod tests {
    ...
    fn extract_json_handles_pure_json() {
        let s = r#"{"case_no": "test"}"#;
        assert_eq!(extract_json_from_content(s), s);
    }

    #[test]
    fn extract_json_strips_markdown_fences() {
        let s = "```json\n{\"case_no\": \"test\"}\n```";
        let out = extract_json_from_content(s);
    ...
    fn extract_json_strips_think_block() {
        let s = "<think>嗯,这个案件是...</think>\n{\"case_no\": null, \"court\": \"X 法院\"}";
        let out = extract_json_from_content(s);
        assert!(out.starts_with('{'));
    ...
    #[test]
    fn extract_json_handles_prefix_text() {
        let s = "结果如下:{\"case_no\": null}";
        let out = extract_json_from_content(s);
        assert_eq!(out, r#"{"case_no": null}"#);
    }

    #[test]
    fn parse_extracted_fields_from_sample() {
        let json = r#"{"case_no": null, "court": "X 法院", "plaintiffs": ["张三"], "defendants": ["李四"], "cause": "民间借贷纠纷", "claim_amount": 50000, "filed_at": "2025-04-15"}"#;
        let f: ExtractedFields = serde_json::from_str(json).unwrap();
        assert_eq!(f.case_no, None);
        assert_eq!(f.court.as_deref(), Some("X 法院"));
        assert_eq!(f.plaintiffs, vec!["张三"]);
        assert_eq!(f.defendants, vec!["李四"]);
        assert_eq!(f.cause.as_deref(), Some("民间借贷纠纷"));
        assert_eq!(f.claim_amount, Some(50000.0));
        assert_eq!(f.filed_at.as_deref(), Some("2025-04-15"));
    }

    #[test]
    fn parse_extracted_fields_with_null_arrays() {
        // 2026-05-31 真机回归:flash 对空数组字段返回显式 null(键在、值 null),
        // struct 的 #[serde(default)] 兜不住 → 以前直接 "invalid type: null" 抽取失败
        // (离婚补偿协议.pdf)。现在 deserialize_null_default 应把 null 兜成空 Vec。
        let json = r#"{
            "case_no": null, "court": "X 法院", "cause": "离婚纠纷",
            "plaintiffs": null, "defendants": null, "third_parties": null,
            "party_contacts": null, "fees": null, "judges": null,
            "court_contacts": null, "key_dates": null, "preservations": null,
            "claim_amount": null, "filed_at": null
        }"#;
        let f: ExtractedFields = serde_json::from_str(json).expect("null 数组不能再让抽取失败");
        assert!(f.plaintiffs.is_empty());
        assert!(f.defendants.is_empty());
        assert!(f.party_contacts.is_empty());
        assert!(f.key_dates.is_empty());
        assert!(f.preservations.is_empty());
        assert_eq!(f.court.as_deref(), Some("X 法院"));
    }

    #[test]
    fn parse_extracted_fields_with_missing_optional_keys() {
        // 模型可能漏字段,#[serde(default)] 应该让所有字段都能默认空
        let json = r#"{"case_no": "(2024)苏02民初1号"}"#;
        let f: ExtractedFields = serde_json::from_str(json).unwrap();
        assert_eq!(f.case_no.as_deref(), Some("(2024)苏02民初1号"));
        assert!(f.plaintiffs.is_empty());
        assert!(f.claim_amount.is_none());
    }

    #[test]
    fn claim_amount_accepts_number_string_or_null() {
        // 数字
        let a: ExtractedFields = serde_json::from_str(r#"{"claim_amount": 50000}"#).unwrap();
        assert_eq!(a.claim_amount, Some(50000.0));

        // 数字字符串
        let b: ExtractedFields = serde_json::from_str(r#"{"claim_amount": "50000"}"#).unwrap();
        assert_eq!(b.claim_amount, Some(50000.0));

        // 带千分位逗号的字符串
        let c: ExtractedFields = serde_json::from_str(r#"{"claim_amount": "50,000.00"}"#).unwrap();
        assert_eq!(c.claim_amount, Some(50000.0));

        // null
        let d: ExtractedFields = serde_json::from_str(r#"{"claim_amount": null}"#).unwrap();
        assert_eq!(d.claim_amount, None);

        // 字段缺失
        let e: ExtractedFields = serde_json::from_str(r#"{}"#).unwrap();
        assert_eq!(e.claim_amount, None);
    }

    /// 端到端集成测试:需要真实的 llama.cpp + MiniCPM-V 4.6 跑在 :8899。
    ///
    /// 默认 `cargo test` 不跑,手工触发:
    ///   `cargo test -p caseboard llm::tests::e2e -- --ignored --nocapture`
    ///
    /// 测试数据用纯虚构姓名(张三/李四 + 5 万元),不含任何真实当事人信息。
    #[tokio::test]
    #[ignore]
    async fn e2e_extract_from_fictional_complaint() {
        let sample = r#"民事起诉状

原告:张三,男,1980 年 1 月 1 日生,汉族,住江苏省无锡市惠山区。
被告:李四,男,1975 年 5 月 5 日生,汉族,住江苏省南京市鼓楼区。

诉讼请求:
1. 判令被告李四偿还原告借款本金人民币 50,000 元;
2. 判令被告承担本案诉讼费用。

事实与理由:
2024 年 3 月 5 日,被告向原告借款人民币 50,000 元,约定借期一年。借期届满后,被告经原告多次催讨,仍未按约定偿还借款。
...
2025 年 4 月 15 日"#;
        ...
        let config = LlmConfig::local_llamacpp_default();
        let result = extract_case_fields(&config, sample).await;
        ...
        let f = result.expect("LLM 应该能正常返回(确认 llama.cpp 在 :8899 跑着)");
        assert!(
            f.plaintiffs.contains(&"张三".to_string()),
            "应识别原告 张三"
        );
        assert!(
            f.defendants.contains(&"李四".to_string()),
            "应识别被告 李四"
        );
        assert_eq!(f.claim_amount, Some(50000.0), "诉讼金额应是 50000");
        assert_eq!(f.filed_at.as_deref(), Some("2025-04-15"));
    }
}
