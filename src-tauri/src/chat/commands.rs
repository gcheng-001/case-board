...
mod tests {
    ...
    #[test]
    fn resolve_tool_choice_always_auto_for_v4_thinking_models() {
        // 实测 2026-05-30:DeepSeek V4 全系(flash/pro/pro-thinking)都是思考模式,都拒绝
        // tool_choice="required"(HTTP 400)。故一律 auto,靠宪法"工具优于直答"驱动工具调用。
        // (旧测试曾断言 flash 应得 "required" —— 那是按模型名判定的错误假设,已被实测推翻。)
        assert_eq!(resolve_tool_choice(true, "deepseek-v4-pro"), "auto");
        assert_eq!(
            resolve_tool_choice(true, "deepseek-v4-pro-thinking"),
            "auto"
        );
        assert_eq!(resolve_tool_choice(true, "deepseek-v4-flash"), "auto");
        assert_eq!(resolve_tool_choice(false, "deepseek-v4-flash"), "auto");
        assert_eq!(resolve_tool_choice(false, "deepseek-v4-pro"), "auto");
    }
    ...
    async fn clear_history_round_trip() {
        ...
        .await
        .unwrap();
        let n = clear_chat_history_impl(&pool, &case.id).await.unwrap();
        assert_eq!(n, 1);
        let after = list_chat_history_impl(&pool, &case.id, None).await.unwrap();
        assert!(after.is_empty());
    }

    #[test]
    fn cancel_registry_take_consumes() {
        let reg = ChatCancelRegistry::default();
        let (tx, mut rx) = oneshot::channel::<()>();
        reg.register("msg-1".into(), tx);
        let cancelled = cancel_chat_impl(&reg, "msg-1");
        assert!(cancelled);
        // 再 cancel 一次应失败(已被 take)
        let cancelled_again = cancel_chat_impl(&reg, "msg-1");
    ...
    #[test]
    fn clip_history_obeys_budget_and_skips_errors() {
        let now = "2026-05-27T00:00:00Z".to_string();
        let mk = |id: &str, role: &str, content: &str, err: Option<&str>| ChatMessage {
            id: id.into(),
            case_id: "c".into(),
            role: role.into(),
        ...
        let history = vec![
            mk("1", "user", "第一问", None),
            mk("2", "assistant", "第一答", None),
            mk("3", "user", "第二问", None),
            mk("4", "assistant", "", Some("HTTP 429")), // error,应跳过
            mk("5", "user", "第三问", None),
            mk("6", "assistant", "第三答", None),
        ];
        let clipped = clip_history_for_replay(&history, 100);
        // 应包含正常 5 条(跳过错误),按正序
        assert!(clipped.iter().all(|(_, c)| !c.is_empty()));
        assert_eq!(clipped.first().map(|x| x.1.as_str()), Some("第一问"));
        assert_eq!(clipped.last().map(|x| x.1.as_str()), Some("第三答"));
    }

    #[tokio::test]
    async fn write_chat_artifact_creates_document_row() {
        // 用临时 HOME 防污染本机
        let _env_guard = crate::chat::HOME_ENV_LOCK.lock().await;
        ...
        let case = create_case(
            ...
            NewCase {
                ...
                case_type: "诉讼".into(),
                source_folder: "/tmp/artifact".into(),
            },
        )
        .await
        .unwrap();

        let doc_id = write_chat_artifact(
            &pool,
            &case.id,
            "assistant-msg-1",
        ...
        // documents 表有这行,且 source='chat'
        let (source, is_ai_artifact, filename): (String, bool, String) =
            sqlx::query_as("SELECT source, is_ai_artifact, filename FROM documents WHERE id = ?")
                .bind(&doc_id)
                .fetch_one(&pool)
                .await
                .unwrap();
        assert_eq!(source, "chat");
        assert!(is_ai_artifact);
        // 可读文件名:中文任务名 + 时间戳,不再是 task_type__uuid 乱码
        assert!(filename.starts_with("案件总览_"), "filename={}", filename);
        assert!(filename.ends_with(".md"));
    }

    #[test]
    fn sanitize_error_truncates_and_scrubs_paths() {
        let raw = "Failed at /Users/leo/secret/path/file.md with code 500";
        let scrubbed = sanitize_error(raw);
        assert!(!scrubbed.contains("/Users/leo/secret"));
        assert!(scrubbed.len() <= raw.len() + 16); // 脱敏后不应膨胀
    }

    /// 隐私铁律 #3 回归测试:
    /// chat_messages.content **永远不进**反馈 MD。
    /// 即使 DB 里有 chat 行,feedback::collect() + render_md() 也必须 100% 不引用它。
    #[tokio::test]
    async fn chat_content_never_leaks_into_feedback_md() {
        let pool = init_pool(":memory:").await.unwrap();
        let case = create_case(
            &pool,
            NewCase {
                name: "feedback_leak_test".into(),
                case_type: "诉讼".into(),
                source_folder: "/tmp/leak".into(),
            },
        )
        .await
        .unwrap();

        // 故意写一段含敏感关键词的 chat 内容
        let secret = "客户银行卡号 6225-1234-5678-9012";
        insert_chat_message(
            &pool,
            NewChatMessage {
                id: "leak-test",
        ...
        // 跑 feedback collect + render
        let info = crate::feedback::collect(&pool, vec![]).await.unwrap();
        let md = crate::feedback::__test_render_md(&info, "用户反馈测试");

        assert!(
            !md.contains(secret),
            "chat 内容泄漏到反馈 MD!\nMD 内容(末尾):\n{}",
            &md.chars().rev().take(500).collect::<String>()
        );
        assert!(!md.contains("6225"), "chat 内容里的卡号片段泄漏到反馈 MD!");
    }
}
