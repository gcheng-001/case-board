...
mod tests {
    ...
    #[test]
    fn parse_ask_user_args_extracts_questions_with_options() {
        let args = serde_json::json!({
            "questions": [
                {"question": "原告是个人还是公司?", "options": ["个人", "公司"], "allow_input": false},
                {"question": "被告全称?", "allow_input": true}
            ]
        });
        let qs = parse_ask_user_args(&args);
        assert_eq!(qs.len(), 2);
        assert_eq!(qs[0].question, "原告是个人还是公司?");
        assert_eq!(qs[0].options, vec!["个人", "公司"]);
        assert!(!qs[0].allow_input);
        // 第二条无 options → 空 vec;allow_input=true
        assert!(qs[1].options.is_empty());
        assert!(qs[1].allow_input);
    }

    #[test]
    fn parse_ask_user_args_defends_against_garbage() {
        // 缺 questions → 空
        assert!(parse_ask_user_args(&serde_json::json!({})).is_empty());
        // questions 不是数组 → 空
        assert!(parse_ask_user_args(&serde_json::json!({"questions": "x"})).is_empty());
        // 条目缺 question / question 空白 → 跳过
        let args = serde_json::json!({
            "questions": [
                {"options": ["a"]},
                {"question": "   "},
        ...
        let qs = parse_ask_user_args(&args);
        assert_eq!(qs.len(), 1);
        assert_eq!(qs[0].question, "有效问题?");
    }

    #[test]
    fn force_finish_prompt_keeps_anti_fabrication() {
        // 达 LoopGuard 上限的强制收尾轮必须守住反虚构底线:没查全的要明说,绝不编造。
        // 防止后续有人精简这条 prompt 时丢掉法律 App 的核心安全约束(隐私铁律 §4)。
        assert!(FORCE_FINISH_PROMPT.contains("严禁编造"));
        assert!(FORCE_FINISH_PROMPT.contains("未能核实"));
        assert!(FORCE_FINISH_PROMPT.contains("最大检索轮数"));
    }
    ...
    #[test]
    fn streaming_tool_call_merges_multi_chunks() {
        let mut sc = StreamingToolCall::default();
        sc.merge(&StreamToolCallDelta {
            index: 0,
            id: Some("call_abc".into()),
            _ty: Some("function".into()),
            function: Some(StreamFunctionDelta {
                name: Some("search_laws".into()),
                arguments: Some("{\"keyword".into()),
            }),
        });
        sc.merge(&StreamToolCallDelta {
            index: 0,
        ...
        sc.merge(&StreamToolCallDelta {
            ...
            function: Some(StreamFunctionDelta {
                name: None,
                arguments: Some(",\"top_k\":10}".into()),
            }),
        });
        let fc = sc.build().unwrap();
        assert_eq!(fc.id, "call_abc");
        assert_eq!(fc.name, "search_laws");
        assert_eq!(fc.args["keyword"], "合同解除");
        assert_eq!(fc.args["top_k"], 10);
    }

    #[test]
    fn streaming_tool_call_empty_arguments_yields_empty_object() {
        let mut sc = StreamingToolCall::default();
        sc.merge(&StreamToolCallDelta {
            index: 0,
    ...
    #[test]
    fn streaming_tool_call_missing_id_errors() {
        let mut sc = StreamingToolCall::default();
        sc.merge(&StreamToolCallDelta {
            index: 0,
            id: None,
    ...
    #[test]
    fn streaming_tool_call_unrepairable_json_falls_back_to_empty_object() {
        // strategy A(arg_repair):流式损坏、连修复阶梯都救不回的参数 → 兜底空对象派发,
        // **绝不炸整轮**。由工具自身校验空参数再报错,体验远好于整条回复崩掉。
        let mut sc = StreamingToolCall::default();
        sc.merge(&StreamToolCallDelta {
            index: 0,
            id: Some("c1".into()),
        ...
        let built = sc
            .build()
            .expect("strategy A:损坏 JSON 不再报错,兜底为空对象");
        assert_eq!(built.args, serde_json::json!({}));
    }

    #[test]
    fn handle_sse_event_accumulates_content() {
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        let mut content = String::new();
        let mut reasoning = String::new();
        let mut tc = HashMap::new();
        let mut fr = None;
        let mut u = ChunkUsage::default();
        let raw = r#"data: {"choices":[{"delta":{"content":"你好"},"index":0}]}"#;
        let stop = handle_sse_event(
            raw,
            &tx,
            &mut content,
            &mut reasoning,
        ...
        let _ = rx.try_recv().unwrap(); // Delta 事件
    }

    #[test]
    fn handle_sse_event_captures_tool_calls_delta() {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        let mut content = String::new();
        let mut reasoning = String::new();
        let mut tc: HashMap<u32, StreamingToolCall> = HashMap::new();
        let mut fr = None;
        let mut u = ChunkUsage::default();
        let raw = r#"data: {"choices":[{"delta":{"tool_calls":[{"index":0,"id":"c1","type":"function","function":{"name":"search_laws","arguments":"{\"keyword\":\"a\"}"}}]}}]}"#;
        handle_sse_event(
            raw,
            &tx,
            &mut content,
        ...
        assert!(tc.contains_key(&0));
        let built = tc.remove(&0).unwrap().build().unwrap();
        assert_eq!(built.name, "search_laws");
    }

    #[test]
    fn handle_sse_event_captures_finish_reason() {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        let mut content = String::new();
        let mut reasoning = String::new();
        let mut tc = HashMap::new();
        let mut fr = None;
        let mut u = ChunkUsage::default();
        let raw = r#"data: {"choices":[{"delta":{},"finish_reason":"tool_calls","index":0}]}"#;
        handle_sse_event(
            raw,
            &tx,
            &mut content,
    ...
    #[test]
    fn handle_sse_event_captures_cache_tokens() {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        let mut content = String::new();
        let mut reasoning = String::new();
        let mut tc = HashMap::new();
        let mut fr = None;
        let mut u = ChunkUsage::default();
        let raw = r#"data: {"choices":[{"delta":{},"finish_reason":"stop","index":0}],"usage":{"prompt_tokens":12345,"completion_tokens":234,"prompt_cache_hit_tokens":11800,"prompt_cache_miss_tokens":545},"model":"deepseek-v4-pro"}"#;
        handle_sse_event(
            raw,
            &tx,
            &mut content,
        ...
        assert_eq!(u.prompt_tokens, Some(12345));
        assert_eq!(u.cache_hit_tokens, Some(11800));
        assert_eq!(u.cache_miss_tokens, Some(545));
    }

    #[test]
    fn handle_sse_event_done_marker() {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        let mut content = String::new();
        let mut reasoning = String::new();
        let mut tc = HashMap::new();
    ...
    #[test]
    fn handle_sse_event_accumulates_reasoning_content() {
        // 回归(2026-05-28):thinking 模型工具调用必须回传 reasoning_content,否则 DeepSeek 400
        // "The `reasoning_content` in the thinking mode must be passed back to the API."。
        // 前置:流式必须把 reasoning_content delta 累积起来(旧代码直接丢弃 → 回传时为空)。
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        let mut content = String::new();
        let mut reasoning = String::new();
        let mut tc = HashMap::new();
        let mut fr = None;
        let mut u = ChunkUsage::default();
        let raw1 = r#"data: {"choices":[{"delta":{"reasoning_content":"先查日期"},"index":0}]}"#;
        let raw2 = r#"data: {"choices":[{"delta":{"reasoning_content":",再查天气"},"index":0}]}"#;
        handle_sse_event(
            raw1,
            &tx,
            &mut content,
        ...
        assert_eq!(reasoning, "先查日期,再查天气");
        // reasoning_content 不进 content,也不发 Delta 事件
        assert!(content.is_empty());
    }

    #[test]
    fn assistant_with_tool_calls_serializes_reasoning_content() {
        // thinking 工具调用轮:reasoning_content 必须出现在回传 JSON 里
        let with_reasoning = ApiMessage::AssistantWithToolCalls {
            role: "assistant".into(),
            content: None,
            reasoning_content: Some("我的思维链".into()),
            tool_calls: vec![ApiToolCall {
                id: "c1".into(),
        ...
        let json = serde_json::to_string(&with_reasoning).unwrap();
        assert!(json.contains("\"reasoning_content\":\"我的思维链\""));

        // 非 thinking(flash)或无思维链:字段省略,不发空 reasoning_content
        let no_reasoning = ApiMessage::AssistantWithToolCalls {
            role: "assistant".into(),
            content: Some("hi".into()),
            reasoning_content: None,
            tool_calls: vec![],
        };
        let json = serde_json::to_string(&no_reasoning).unwrap();
