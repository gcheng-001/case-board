-- 0041 · 案件 AI 助手记忆表自愈(fixup)
--
-- 背景:0f27fd8(sync: v0.4.1)上游合并引发迁移编号冲突 ——
-- 旧库 version 37 这个槽位早已被 "document tags compat" 占用,而本仓把 0037 编号给了
-- case_memories;叠加 db/mod.rs::reconcile_migration_checksums 会无差别对齐校验值,
-- 导致 sqlx 误判 0037 "已应用"而跳过建表 → 运行时报 no such table: case_memories。
--
-- 解法:用一个全新的 version 41 重新建表(全部 CREATE TABLE IF NOT EXISTS,幂等):
--   * 受影响的旧机器 → 重启应用即自动补建,自愈;
--   * 全新库         → 0037 已建过本组表,本迁移幂等跳过;
--   * 已手动修复的机器 → 表已存在,幂等无害。
--
-- (建表 SQL 与 0037_case_memories.sql 完全一致,独立编号仅为绕开 37 撞号。)
