-- 0043 · AI 作业与分析陈旧标记基础设施(本地 fork 重编号,原 upstream 0039)
--
-- ⚠️ 重编号说明(2026-07-07 · v0.4.6 合并):upstream v0.4.5 此迁移编号为 0039,
-- 但本地 fork 已有 0039_court_filing_jobs_cascade 占位迁移(老用户 _sqlx_migrations
-- 已记录 version=39)。若保留 0039 会触发 UNIQUE constraint 冲突导致启动闪退,
-- 故本地将本迁移改为 0043。新装用户从 0001 顺序应用无影响;老用户 version 39 已
-- 是 cascade 占位,本迁移作为 0043 新应用,建 ai_jobs / analysis_* 等表字段。
--
-- 目标:
