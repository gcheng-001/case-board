...
class NedevScript(OAScriptBase):
    ...
    def run_approval_action(
        ...
        force_approve = bool(approval_options.get("force_approve"))
        if approved and gate_errors and not force_approve:
            return OAResult(success=False, message="审批通过被门禁阻止", data=preview)
        if approved and gate_errors and force_approve:
            preview["gate_overridden"] = True
            preview["override_reason"] = "partner_final_approval"
        if not confirm:
    ...
    def _approval_case_summary(self, detail: dict[str, Any], entity: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "case_type_display": " / ".join(x for x in (self._text(base_type), self._text(case_category)) if x),
            "charge_method": charge_method,
            ...
            "risk_charge_terms": self._first_text(
                detail,
                entity,
                "riskChargeMemo",
                "RiskChargeMemo",
    ...
    def _get_case_entity_for_approval(self, lawcase_id: int, detail: dict[str, Any]) -> dict[str, Any] | None:
        ...
        elif any(keyword in base_type for keyword in ("非诉", "法律顾问", "顾问", "调查")):
            preferred.extend(
                [
                    "Page:LawcaseDetails_非诉@1",
                    "Page:LawcaseDetails_法律顾问@1",
                    "Page:LawcaseDetails_顾问@1",
                    "Page:LawcaseDetails_调查@1",
                ]
            )
        else:
        ...
        owners = preferred + [
            "Page:LawcaseDetails_民事@1",
            "Page:LawcaseDetails_刑事@1",
            "Page:LawcaseDetails_行政@1",
            "Page:LawcaseDetails_非诉@1",
            "Page:LawcaseDetails_法律顾问@1",
            "Page:LawcaseDetails_顾问@1",
            "Page:LawcaseDetails_调查@1",
            "Page:LawcaseDetails_仲裁@1",
            "Page:LawcaseDetails_执行@1",
            "Page:LawcaseDetails@1",
        ]
        seen: set[str] = set()
        errors: list[str] = []
        for owner in owners:
            ...
            seen.add(owner)
            try:
                entity = self._get_entity("ApplicationData.Lawcase", [lawcase_id], [owner])
                if entity:
                    return entity
            except Exception as exc:
                errors.append(f"{owner}: {exc}")
        if errors:
            self.report("approval_detail_entity_unavailable", 35, "未能读取案件详情实体，已使用 AgentAPI 摘要继续复核", {"errors": errors[:5]})
        return None
