...
export function OAApprovalModule() {
  ...
  const submitApprove = async () => {
    ...
    if (!confirm(`确认通过 ${caseNo(selected)}？`)) return;
    try {
      const s = await oaApproveCase(configId, credentialId, selectedLawcaseId, memo.trim(), approvalOptions());
      setActionSession(s);
                  ...
                  {Number(selected.status) !== 4 && (
                    <div className="space-y-3 rounded border border-border bg-background p-4">
                      ...
                      <div className="flex gap-2">
                        <Button onClick={submitApprove} disabled={!canApprove || missingManualInput}>
